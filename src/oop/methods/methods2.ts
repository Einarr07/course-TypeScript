...
function handShake(name: string, hand?: string): string {
...
function additionAlls(...numbers: number[]): number {

    // The reduce() method is used to sum
    // all values inside the array.
    return numbers.reduce(
        (accumulated, actual) => accumulated + actual,
        0
    );
}
...
function goodBye(name: string, sayBye: string = 'Good bye'): string {
    return `${sayBye}, ${name}`;
}
